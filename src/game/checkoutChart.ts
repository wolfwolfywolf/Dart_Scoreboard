...
/**
 * The standard double-out checkout chart, as printed on most pub charts.
 * Keys are the score remaining; values are the recommended darts.
 * Scores with no entry (169, 168, 166, 165, 163, 162, 159 and anything over 170)
 * cannot be finished in three darts.
 */
...
export const CHART_MAX = 170;

/** Parse a chart label like "T20", "D16", "25", "Bull" or "7" into a dart. */
export function parseDartLabel(label: string): Dart {
  if (label === 'Bull') return { v: 25, m: 2 };
  if (label === '25') return { v: 25, m: 1 };
  if (label[0] === 'T') return { v: Number(label.slice(1)), m: 3 };
  if (label[0] === 'D') return { v: Number(label.slice(1)), m: 2 };
  return { v: Number(label), m: 1 };
}

export function chartRoute(remaining: number): Dart[] | null {
  const entry = CHECKOUT_CHART[remaining];
  return entry ? entry.split(' ').map(parseDartLabel) : null;
}

export function chartTotal(route: Dart[]): number {
